...
def value(bag1):
    total = 0
    for i,j in bag1.items():
        total = total + (i*j)
    return(total)
        


def transfer1coin(bag1, bag2):
    global c
    c = int(input("qual o tipo da moeda que quer transferir?"))
    if bag1[c] != 0:
        return True
...
def main():
    ...
    bag2 = {1: 2, 2: 10, 5:5, 10: 2, 20: 9, 50: 2, 100: 0, 200: 5}

    total = value(bag1)
    print("o valor da carteira é: ",total)

    transf = transfer1coin(bag1, bag2)
    if transf:
        bag2[c] += 1
        bag1[c] -= 1
        print(bag1)
