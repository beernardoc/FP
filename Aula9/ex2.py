...
numJogos = lambda reg: reg[1]+reg[2]+reg[3]
...
def main():
    ...
    tab = sorted(tabela, key=lambda tabela: (tabela[4]-tabela[5]), reverse=True)
    print(tab)
    ...
    tab = sorted(tabela, key= lambda tabela: (pontos(tabela), tabela[GM] - tabela[GS]), reverse = True) 
    print("aq")
    printTabela(tab)
